...
export async function POST(request: NextRequest) {
  ...
  const modifierPrompts: Record<string, string> = {
    ...
    "80s-cave": "ake the uploaded image as the base reference and transform it into a high-quality cross-section illustration of a layered cake, while preserving the original composition, perspective, and overall shape.Visu ally slice the cake cleanly to reveal its interior layers, showcasing realistic textures and depth. Render multiple distinct layers with clear separation and accurate material detail, such as:– vanilla sponge (light, airy crumb)– chocolate ganache (smooth, glossy, dense)– raspberry jam (translucent, jewel-toned)– lemon curd (silky, vibrant yellow)– matcha mousse (soft, pale green, creamy)Label each layer directly with clean, modern typography and subtle leader lines. All labels must remain legible at small / thumbnail scale. Add a small interactive-style overlay or legend positioned to the side or corner of the image that visually suggests selectable flavor layers and quantities (toggle-style icons, checkmarks, or sliders), as if the user can customize which layers are active. This overlay should look UI-inspired but remain part of the illustration.Lighting should be soft and appetizing, emphasizing moisture, texture, and contrast between layers. Use a polished editorial illustration style suitable for a premium bakery or product configurator.Maintain realism, clarity, and visual hierarchy. Do not clutter the image. Prioritize clarity of layers and labels over decoration.",  //"Generate a detailed cross-section illustration of the cake showing interior layers and fillings. Render labelled flavor layers (e.g., vanilla sponge, chocolate ganache, raspberry jam, lemon curd, matcha mousse) with realistic textures. Also include a small interactive-style overlay or legend that visually highlights selectable flavor options and quantities, as if the user can toggle layers. Maintain overall composition and make labels legible at thumbnail scale.",
